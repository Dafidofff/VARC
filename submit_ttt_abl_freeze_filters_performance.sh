...
#SBATCH --job-name=ttt_abl_freeze
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=0-04:00:00
#SBATCH --mem=64G
#SBATCH --output=logs/ttt_abl_freeze_filters_perf_%A_%a.out
#SBATCH --array=0-7

# H2 ablation: freeze Hyena conv filter weights (global_conv.kernel, short_conv, shortcut)
# during TTT, leaving FFN, projections, and norms trainable.
# Hypothesis: Hyena conv filters encode global task-agnostic patterns that should not
# be disturbed by the small TTT dataset; fine-tuning only the task-specific components
# may yield faster, more stable adaptation.
# Best config from prior ablations: LR=1e-3, constant schedule, 100 epochs.

echo "TTT abl freeze_filters array ${SLURM_ARRAY_TASK_ID}/7 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
...
EVAL_SAVE_NAME="ttt_ablation/freeze_filters"
STRIDE=8
