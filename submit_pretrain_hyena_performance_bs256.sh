...
mkdir -p logs saves/offline_train_Hyena_bs256 wandb

MASTER_PORT=$(( 29500 + (SLURM_JOB_ID % 1000) ))

# Performance partition: 4×RTX 6000 Ada.
...
torchrun --nproc_per_node=4 --master-port=$MASTER_PORT offline_train_ARC.py \
  ...
  --wandb-run-name "varc_hyena_bs256_performance" \
  --save-path "saves/offline_train_Hyena_bs256/checkpoint_final.pt" \
  --best-save-path "saves/offline_train_Hyena_bs256/checkpoint_best.pt" \
  --lr-scheduler "cosine" \
  --architecture "hyena" \
