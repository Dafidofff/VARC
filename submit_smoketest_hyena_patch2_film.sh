...
#SBATCH --job-name=smoke_p2_film
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=0:30:00
#SBATCH --mem=32G
#SBATCH --output=logs/smoketest_hyena_patch2_film_%A.out
...
mkdir -p logs saves/smoketest_Hyena_patch2_film
...
torchrun --nproc_per_node=1 --master-port=$MASTER_PORT offline_train_ARC.py \
  ...
  --save-path "saves/smoketest_Hyena_patch2_film/checkpoint_final.pt" \
  --best-save-path "saves/smoketest_Hyena_patch2_film/checkpoint_best.pt" \
  --lr-scheduler "cosine" \
  --architecture "hyena" \
