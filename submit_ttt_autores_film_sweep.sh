...
# PARAMETERIZED hparam-sweep harness for the BEST checkpoint (FiLM+BlockDiag p=2).
# Phase 2 of the loop: re-run the standard ablations on the 43% checkpoint, since the
# earlier sweep was done on the 36% BlockDiag baseline (a different, under-fitting regime).
# Base recipe = film_unfrozen (FiLM+BlockDiag ckpt, UNFROZEN, lr1e3_const), confirmed 42.75%
# (tied with frozen 43.25%); we vary ONE hparam per launch.
#
# Launch via sbatch --export, overriding job-name/output on the CLI, e.g.:
#   sbatch -J ttt_film_lr7e4 -o logs/ttt_autores_film_lr7e4_%A_%a.out \
#          --export=ALL,SWEEP_NAME=lr7e4,LR=7e-4 submit_ttt_autores_film_sweep.sh
...
#
# Tunable env vars (defaults = the film_unfrozen base recipe):
#   SWEEP_NAME (required)  LR=1e-3  SCHED=none  WD=0  NUM_ATTEMPTS=10  EPOCHS=100
#   TTT_NUM_EACH=2  FREEZE=0  BATCH=8

echo "TTT autores film sweep '${SWEEP_NAME}' array ${SLURM_ARRAY_TASK_ID}/24 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
...
echo "Recipe: LR=$LR SCHED=$SCHED WD=$WD NUM_ATTEMPTS=$NUM_ATTEMPTS EPOCHS=$EPOCHS TTT_NUM_EACH=$TTT_NUM_EACH FREEZE=$FREEZE BATCH=$BATCH"
