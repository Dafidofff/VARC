...
# Full-400 TTT for Hybrid HHAA (paired H H A A H H A A...) checkpoint.
# Recipe: LoRA r4 on qkv_proj/out_proj (base frozen), lr1e3_const.
# Requires rope.py fix (inplace ops on gradient views crash TTT backward).
# Output dir: ttt_autores/film_hhaa_r4  (reuses prior partial outputs via skip-if-done)

echo "TTT hybrid HHAA LoRA r4 400 array ${SLURM_ARRAY_TASK_ID}/39 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
...
LORA_RANK=4
LORA_ALPHA=4
HYENA_CONFIG="/home/dwessel/code/nvSubquadratic-private/varc_configs/cfg_hybrid_hhaa_p2_blockdiag_film.py"
CKPT="saves/offline_train_Hyena_hybrid_hhaa_p2_blockdiag_film/checkpoint_best.pt"
EVAL_SAVE_NAME="ttt_autores/film_hhaa_r4"
STRIDE=40
