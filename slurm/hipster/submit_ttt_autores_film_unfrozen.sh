...
#SBATCH --job-name=ttt_autores_filmunfrz
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=0-04:00:00
#SBATCH --mem=32G
#SBATCH --output=logs/ttt_autores_film_unfrozen_%A_%a.out
#SBATCH --array=0-24
#SBATCH --exclude=hipster-cn011  # broken node: /local_scratch container ns missing -> launch failures

# Autoresearch experiment #17 — ABLATION of the first confirmed win (exp#14 film_freeze).
# film_freeze = BlockDiag+FiLM+AdaLN p=2 ckpt + --freeze-hyena-filters scored 43.25% on 400.
# That combines TWO changes vs baseline: (a) the FiLM checkpoint (more conditioning capacity),
# (b) freezing the filters. This run isolates them: SAME FiLM ckpt, UNFROZEN (no freeze flag),
# baseline lr1e3_const. If unfrozen >= frozen, the win is the FiLM capacity and freeze is
# unnecessary; if frozen > unfrozen, freeze also matters on this representation.
# Note: on the plain BlockDiag p=2 ckpt, freeze ~= unfrozen on 400 (both 36%); FiLM kernels
# are structurally different, so the answer is genuinely open.
# 25-way array, STRIDE=25 -> 4 tasks/job, fits the 4h wall. Skip-if-done makes requeue safe.
# Score with: python scripts/score_one.py ttt_autores/film_unfrozen

echo "TTT autores film_unfrozen array ${SLURM_ARRAY_TASK_ID}/24 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
