...
#SBATCH --job-name=varc_hyena_p2_blkd
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=128
#SBATCH --time=3-00:00:00
#SBATCH --mem=128G
#SBATCH --output=logs/varc_hyena_patch2_blockdiag_%A.out
...
mkdir -p logs saves/offline_train_Hyena_patch2_blockdiag_lr1e3 wandb
...
torchrun --nproc_per_node=4 --master-port=$MASTER_PORT offline_train_ARC.py \
  ...
  --wandb-run-name "varc_hyena_patch2_blockdiag_lr1e3" \
  --save-path "saves/offline_train_Hyena_patch2_blockdiag_lr1e3/checkpoint_final.pt" \
  --best-save-path "saves/offline_train_Hyena_patch2_blockdiag_lr1e3/checkpoint_best.pt" \
  --lr-scheduler "cosine" \
  --architecture "hyena" \
