...
mkdir -p logs saves/offline_train_ViT_bs256 wandb

# Performance partition: 4×RTX 6000 Ada (49 GB each).
# Effective batch size: 4×64=256, matching the paper's target.
torchrun --nproc_per_node=4 offline_train_ARC.py \
  ...
  --wandb-run-name "varc_pretrain_bs256_performance" \
  --save-path "saves/offline_train_ViT_bs256/checkpoint_final.pt" \
  --best-save-path "saves/offline_train_ViT_bs256/checkpoint_best.pt" \
  --lr-scheduler "cosine" \
  --architecture "vit" \
  --vis-every 50 \
  --distributed \
