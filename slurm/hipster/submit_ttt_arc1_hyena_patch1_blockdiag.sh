...
#SBATCH --job-name=varc_ttt_hyena_p1_blkd
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --time=7-00:00:00
#SBATCH --mem=64G
#SBATCH --output=logs/varc_ttt_hyena_patch1_blockdiag_%A_%a.out
#SBATCH --array=0-39

echo "TTT ARC-1 Hyena patch1 BlockDiag array job ${SLURM_ARRAY_TASK_ID}/39 on node: $SLURM_NODELIST"
echo "Start: $(date)"
...
HYENA_CONFIG="/home/dwessel/code/nvSubquadratic-private/varc_configs/cfg_hyena_rearc_subq_ops_patch1_circular_adaln_blockdiag.py"
CHECKPOINT="saves/offline_train_Hyena_patch1_blockdiag_lr1e3/checkpoint_best.pt"
EVAL_SAVE_NAME="ARC_1_eval_Hyena_patch1_blockdiag_attempt_0"
...
for file_name in "${file_names[@]}"; do
  if (( i % STRIDE == JOB_IDX )); then
    if [ -f "outputs/${EVAL_SAVE_NAME}_attempt_0/${file_name}_predictions.json" ]; then
      echo "[array ${JOB_IDX}] skipping ${file_name} (already done)"
    else
      echo "[array ${JOB_IDX}] processing task ${file_name} (index ${i})"
