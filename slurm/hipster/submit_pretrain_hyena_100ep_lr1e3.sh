...
#SBATCH --job-name=varc_hyena_100ep_lr1e3
#SBATCH --nodes=1
...
mkdir -p logs saves/offline_train_Hyena_100ep_lr1e3 wandb
...
torchrun --nproc_per_node=4 --master-port=$MASTER_PORT offline_train_ARC.py \
  ...
  --wandb-run-name "varc_hyena_100ep_lr1e3" \
  --save-path "saves/offline_train_Hyena_100ep_lr1e3/checkpoint_final.pt" \
  --best-save-path "saves/offline_train_Hyena_100ep_lr1e3/checkpoint_best.pt" \
  --lr-scheduler "cosine" \
  --architecture "hyena" \
