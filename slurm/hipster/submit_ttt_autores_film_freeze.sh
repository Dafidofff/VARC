...
#SBATCH --job-name=ttt_autores_filmfrz
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=0-04:00:00
#SBATCH --mem=32G
#SBATCH --output=logs/ttt_autores_film_freeze_%A_%a.out
#SBATCH --array=0-24

# Autoresearch experiment #14 — FREEZE-FILTERS on the NEW BlockDiag+FiLM+AdaLN p=2 ckpt.
# Checkpoint: saves/offline_train_Hyena_patch2_blockdiag_film_lr1e3/checkpoint_best.pt
# (job 277655, val_acc 81.49%, config ..._blockdiag_film.py) -- the third pretrained
# representation, with SIREN-modulated FiLM kernels (a genuinely different filter family).
# Recipe = baseline lr1e3_const + --freeze-hyena-filters (freezes 180/331 tensors incl.
# the 48 FiLM kernel-generator params). Tests whether freezing helps on THIS representation.
# Context (tracker.md): freezing filters did OPPOSITE things by checkpoint -- hurt p=1
# (under-formed filters), "helped" BlockDiag p=2 on the 50-task subset (+18pp) but that
# vanished at full 400 (36.0% = unfrozen). FiLM kernels are structurally different, so this
# is a real open question, directly probing what the pretrained filters encode.
# 25-way array, STRIDE=25 -> 4 tasks/job, fits the 4h wall. Skip-if-done makes requeue safe.
# Score with: python scripts/score_one.py ttt_autores/film_freeze

echo "TTT autores film_freeze array ${SLURM_ARRAY_TASK_ID}/24 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
