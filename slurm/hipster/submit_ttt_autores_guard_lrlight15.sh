...
#SBATCH --job-name=ttt_autores_g_lrl15
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=0-04:00:00
#SBATCH --mem=32G
#SBATCH --output=logs/ttt_autores_guard_lrlight15_%A_%a.out
#SBATCH --array=0-24

# Autoresearch experiment #15 — COMBINATION: NaN-guard + decoupled light-LR 1.5x.
# On BlockDiag p=2 (saves/offline_train_Hyena_patch2_blockdiag_lr1e3), lr1e3_const.
# The NaN-guard is now baked into the baseline code; this adds --lr-light-mult 1.5.
# Rationale (synergy, not just stacking): exp#10 light-LR 1.5x BEAT the screen (43%) but
# was CONFOUNDED -- it NaN-diverged 22/100 tasks, so the win was untrustworthy. The
# NaN-guard exists precisely to fix that failure mode. Together, the higher light-path LR
# can help WITHOUT the divergence that invalidated it -- the one combination where each
# trick addresses the other's weakness. If screen > 42% -> full-400 confirm vs 36.25%.
# 25-way array, STRIDE=25 -> 4 tasks/job, fits the 4h wall limit. Skip-if-done safe.
# Score with: python scripts/score_one.py ttt_autores/guard_lrlight15

echo "TTT autores guard_lrlight15 array ${SLURM_ARRAY_TASK_ID}/24 on ${SLURM_NODELIST}"
echo "Start: $(date)"
set -eo pipefail
...
EVAL_SAVE_NAME="ttt_autores/guard_lrlight15"
STRIDE=25
