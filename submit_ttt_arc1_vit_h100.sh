...
#SBATCH --gpus=1
#SBATCH --job-name=varc_ttt_arc1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=2-00:00:00
#SBATCH --mem=64G
#SBATCH --output=logs/varc_ttt_arc1_%A.out

echo "TTT ARC-1 job running on node: $SLURM_NODELIST"
echo "Start: $(date)"
...
# Single GPU: all tasks run sequentially on GPU 0.
NUM_GPUS=1
    ...
    for file_name in "${file_names[@]}"; do
      if (( i % NUM_GPUS == gpu )); then
        ...
        CUDA_VISIBLE_DEVICES=${gpu} python test_time_train_ARC.py \
          ...
          --architecture "vit" \
          --eval-save-name "ARC_1_eval_ViT_attempt_0" \
          --num-attempts 10 \
          --ttt-num-each 2
      fi
      i=$((i + 1))
...
wait
echo "All VARC-ViT ARC-1 TTT tasks finished."
echo "Done: $(date)"
